...
class Router : public Node {
  ...
  public:
    ...
    getNextHop(const std::string &dstId) const; // Gets the next hop interface for a destination ID
    void receive(std::shared_ptr<Packet> packet, const std::shared_ptr<Interface> &fromInterface,
                 PacketTimeTracker *tracker) override; // Receives a packet from a specific interface
    void send(std::shared_ptr<Packet> packet, const std::string dstId,
              PacketTimeTracker *tracker) override; // Sends a packet to a specific destination ID
