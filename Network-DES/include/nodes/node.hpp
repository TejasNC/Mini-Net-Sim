...
class Node {
  ...
  public:
    ...
    // Node methods
    virtual void receive(std::shared_ptr<Packet> packet, const std::shared_ptr<Interface> &fromInterface,
                         PacketTimeTracker *tracker) = 0; // Receives a packet from a specific interface
    virtual void send(std::shared_ptr<Packet> packet, const std::string dstId,
                      PacketTimeTracker *tracker) = 0;                 // Sends a packet to a specific destination ID
